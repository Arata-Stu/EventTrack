...
def prophesee_bbox_filter(labels: np.ndarray, dataset_type: str) -> np.ndarray:
    """
    Gen1: バウンディングボックスの対角線の長さが30以上、幅と高さが10以上のバウンディングボックスのみを保持する
    Gen4: バウンディングボックスの対角線の長さが60以上、幅と高さが20以上のバウンディングボックスのみを保持する
...
def get_base_delta_ts_for_labels_us(unique_label_ts_us: np.ndarray, dataset_type: str = 'gen1') -> int:
    ...
    assert hz in {20, 30, 60}, f'{hz=} but should be either 20 or 30 or 60'

    if hz == 60:
        delta_t_us_approx_10hz = int(6 * median_diff_us)
    elif hz == 30:
        delta_t_us_approx_10hz = int(3 * median_diff_us)
    elif hz == 20:
        delta_t_us_approx_10hz = int(2 * median_diff_us)
...
if __name__ == '__main__':
    ...
    def resolve_seq_dirs(seq_names):
        resolved = []
        for name in seq_names:
            path = dataset_input_path / name
            if path.is_dir():
                resolved.append(path)
            else:
                print(f"[!] WARNING: {name} ディレクトリが見つかりません: {path}")
        return resolved

    train_seq_dirs = resolve_seq_dirs(split_names["train"])
    val_seq_dirs = resolve_seq_dirs(split_names["val"])
    test_seq_dirs = resolve_seq_dirs(split_names["test"])

    total = len(train_seq_dirs) + len(val_seq_dirs) + len(test_seq_dirs)
    print(f"Total sequences: {total} (train: {len(train_seq_dirs)}, val: {len(val_seq_dirs)}, test: {len(test_seq_dirs)})")
    ...
    for seq_dir in train_seq_dirs:
        ...
        if data: seq_data_list.append(data)
